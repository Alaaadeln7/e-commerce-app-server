...
export const markAsShipped = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (order.paymentStatus !== "paid") {
      return res.status(400).json({ status: FAILED, message: "Cannot ship unpaid order" });
    }
    ...
    res.json({ status: SUCCESS, message: "Order updated to 'Shipped'", data: order });
  } catch (error) {
...
export const markAsDelivered = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    if (order.orderStatus !== "shipped") {
      return res.status(400).json({ status: FAILED, message: "Cannot deliver unshipped order" });
    }

    order.orderStatus = "delivered";
    await order.save();

    res.json({ status: SUCCESS, message: "Order updated to 'Delivered'", data: order });
  } catch (error) {
